...
class CameraOperation:
    ...
    def Open_device(self):
        if not self.b_open_device:
            ...
            if ret != 0:
                return ret
            print("hik open device successfully!")
    ...
    # 开始取图
    def Start_grabbing(self, winHandle, event):
        if not self.b_start_grabbing and self.b_open_device:
            ...
            try:
                ...
                self.h_thread_handle = threading.Thread(target=CameraOperation.Work_thread, args=(self, winHandle, event))
    ...
    # 软触发一次
    def Trigger_once(self, timestamp):
        if self.b_open_device:
    ...
    # 取图线程函数
    def Work_thread(self, winHandle, event):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                print("hik get one frame: Width[%d], Height[%d], nFrameNum[%d]"
                      % (self.st_frame_info.nWidth, self.st_frame_info.nHeight, self.st_frame_info.nFrameNum))
                # D_file.write("get one frame: Width[%d], Height[%d], nFrameNum[%d]"
                #       % (self.st_frame_info.nWidth, self.st_frame_info.nHeight, self.st_frame_info.nFrameNum))
                # 释放缓存
    ...
    #     if len(paths) != 0:
    #         if int(paths[-1][:6]) >= self.idx:
    #             self.idx = int(paths[-1][:6]) + 1

    def Save_np_jpg(self, timestamp):
        save_dir = "./data/hik_img/"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        # self.get_save_id(save_dir)
        characters_to_remove = " :"
        for char in characters_to_remove:
            timestamp = timestamp.replace(char, '-')
        file_path = save_dir + timestamp + ".jpg"
        imwrite(file_path, self.img_np)
    ...
    def Save_jpg(self):
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        file_path = save_dir + f"{time.time()}.jpg"
        ...
        print(f"save_temp_jpg to :{file_path}")
    ...
    def Merge_Down(self):
        if True == self.b_open_device:
            ret = self.obj_cam.MV_CC_SetEnumValue("BinningHorizontal", 2)  # 重点！
            ret = self.obj_cam.MV_CC_SetEnumValue("BinningVertical", 2)  # 重点！
            if ret != 0:
                print('Merge_Down error', 'ret = ' + To_hex_str(ret))
            return ret

    def Set_auto_Exposure(self, mode):
        if True == self.b_open_device:
            ret = self.obj_cam.MV_CC_SetEnumValueByString("ExposureAuto", mode)  # 重点！
            ret = self.obj_cam.MV_CC_SetEnumValueByString("GainAuto", mode)  # 重点！
            if ret != 0:
                print('Set ExposureAuto error ', 'ret = ' + To_hex_str(ret))
            return ret

    def Get_auto_Exposure(self):
        # auto_mode = MV_CAM_EXPOSURE_AUTO_MODE()
        auto_mode = MVCC_ENUMVALUE()
        memset(byref(auto_mode), 0, sizeof(MVCC_ENUMVALUE))
        if True == self.b_open_device:
            ret = self.obj_cam.MV_CC_GetEnumValue("ExposureAuto", auto_mode)  # 重点！
            if ret != 0:
                print('Get ExposureAuto error ', 'ret = ' + To_hex_str(ret))
            return auto_mode.nCurValue
