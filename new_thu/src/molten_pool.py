...
class Ellipse_Detector():
    ...
    def get_major_axis(self, ellipse, original_image,Draw=True, put_text=False):
        # 获取椭圆的短轴和边界的两个交点
        center, axes, angle = ellipse
    ...
    def pre_centor(self):
        ...
        if ellipse1 is None or ellipse2 is None:
            return False, False, False
        self.get_major_axis(ellipse1, img,False)
    ...
    def get_dir_tiff_res(self, dir_path, save_path):
        ...
        for img_file in third_quarter_files:
            ...
            if abs(long_axis - mean_long) <= outlier_threshold * mean_long and \
                ...
                filtered_data.append({"File": img_file, "LongAxis": long_axis, "ShortAxis": short_axis})
        ...
        # 添加均值作为最后一行
        df.loc[len(df)] = {"File": "Mean", "LongAxis": final_mean_long, "ShortAxis": final_mean_short}
        df.to_csv(output_csv, index=False)

        # 绘制统计图并保存为图片
        plt.figure(figsize=(10, 6))
        plt.hist(third_long_axes, bins=15, alpha=0.7, label="Long Axis")
        plt.hist(third_short_axes, bins=15, alpha=0.7, label="Short Axis")
        ...
        plt.legend()
        output_plot = os.path.join(save_path, "axis_statistics.png")
        plt.savefig(output_plot)

        print(f"Filtered data saved to {output_csv}")
        print(f"Axis statistics plot saved to {output_plot}")
...
if __name__ == "__main__":
    ...
    path = r"F:\1106-molten-108-group\centor\centor-93"
    save_path = path + "-res"
